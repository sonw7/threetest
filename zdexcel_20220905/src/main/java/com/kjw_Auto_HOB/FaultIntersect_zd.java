...
public class FaultIntersect_zd {
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0;i<filespathname.length;i++) {
            ...
            for (int s=0;s<MFaults.size();s++){
                ...
                MainFault = SaveFaultinfor(MFpath, MFaults.get(s).MFname);//存储主断层
                MainFault.isMF=true;
            ...
            for (int j = 0; j < faults.size(); j++) {
                num = CountIntersectionNum(MainFault, faults.get(j));//判断num= 4 四个交点 十字形 2 两个交点分支形  1个单交点 0 无交点

                GXnum=CountGXNum(MainFault, faults.get(j),num);//记录在交点再同一个线段 出现几次，处理特殊情况 主要是2和4交点  0和1可不看

                MainFault.vertexes = deleteMFvertex(MainFault, faults.get(j), counterclockwiseV, num,GXnum);//记录需要删除主断层的点

                //函数返回交点和其逆序离散点
                cVertex = handleintersect(MainFault, faults.get(j), num,GXnum);
...
//        Workbook workbook = readExcelAb(MyPath+"\\fayang.xlsx");
        ...
        try {
            FileOutputStream fos = new FileOutputStream(MyPath+"\\"+MFname+"_qiujiao.xlsx");
            workbook.write(fos);
            workbook.close();
            fos.close();
