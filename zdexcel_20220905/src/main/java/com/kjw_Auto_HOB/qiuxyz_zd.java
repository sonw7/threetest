...
/*已知xyz和所求点的Z值，依次求所求点的X Y*/
public class qiuxyz_zd {
...
//        String filename = adress;
        ...
        File file1 = new File("C:\\Users\\86195\\Desktop\\断层求交九煤111901");//所读入的文件夹
        //用数组把文件夹下的文件存起来
        ...
        for(int i=0;i<filesname.length;i++){
            ...
            for (File file3 : files1) {
                //打印文件列表：只读取名称使用getName();
                function(file3.getPath());//进行对应的x y z 求解。
...
//        function(filename);
    ...
    public static void function(String filename) throws IOException {
        ...
        String ctitle[] = new String[4];
        int rows = sheet.getLastRowNum();    //得到该sheet的行数
        String rtitle[] = new String[rows];
        double Originxyz[][] = new double[rows][4];
        ...
        for (int i = 1; i <=rows; i++) {
            ...
            Originxyz[i-1][0] = sxyz[i - 1][0] = cell1.getNumericCellValue();
            cell1 = row.getCell(2);    //表示获取第i行第3列的单元格
            Originxyz[i-1][1] = sxyz[i - 1][1] = cell1.getNumericCellValue();
            cell1 = row.getCell(3);     //表示获取第i行第4列的单元格
            Originxyz[i-1][2] = sxyz[i - 1][2] = cell1.getNumericCellValue();
            cell1 = row.getCell(4);     //表示获取第i行第5列的单元格
            Originxyz[i-1][3] = sxyz[i - 1][3] = cell1.getNumericCellValue();
        }
...
//            yizhiz[i2][1]=sxyz[i2][7];
        ...
        writeTest(jieguo, ban, filename,ctitle,rtitle,Originxyz);
    }

    private static void writeTest(Double[][] jieguo, int count, String MyPath,String[] ctitle,String[] rtitle,double[][] Originxyz ) throws IOException {
...
//        Sheet sheet = workbook.createSheet("Sheel2");
        ...
        for (int i = 0; i < count; i++) {
            ...
            cell.setCellValue(rtitle[i]);
            cell=row0.createCell(1);
            cell.setCellValue(Originxyz[i][0]);
            cell=row0.createCell(2);
            cell.setCellValue(Originxyz[i][1]);
            cell=row0.createCell(3);
            cell.setCellValue(Originxyz[i][2]);
        ...
        for (; i1 >= 0; i1--) {
            ...
            cell=row0.createCell(0);
            cell.setCellValue(rtitle[jilu]);
            cell=row0.createCell(1);
            cell.setCellValue(Originxyz[jilu][0]);
            cell=row0.createCell(2);
            cell.setCellValue(Originxyz[jilu][1]);
            cell=row0.createCell(3);
            cell.setCellValue(Originxyz[jilu][2]);
            cell=row0.createCell(4);
            cell.setCellValue(Originxyz[jilu][3]);
